...
package body AVL_Array_Key_Trees is
   ...
   function Is_Left_Less
     (X : in Key_T;
      Y : in AVL_Node_A)
      return Boolean
   is
   begin
      return Is_Less_Than (X, Y.all.Key);
   end Is_Left_Less;
   pragma Inline (Is_Left_Less);

   function Is_Right_Less
     (X : in AVL_Node_A;
      Y : in Key_T)
      return Boolean
   is
   begin
      return Is_Less_Than (X.all.Key, Y);
   end Is_Right_Less;
   pragma Inline (Is_Right_Less);
   ...
   begin
      loop
         if Is_Left_Less (Key, DNode) then
            ...
            PNode := DNode;
            DNode := DNode.all.LL;
         elsif Is_Right_Less (DNode, Key) then
   ...
   is
      ...
      begin
         loop
            if Is_Left_Less (The_Key, Node) then
               case Node.all.Link_Kind (Left) is
                  when Child =>
                     Node := Node.all.LL;
                  ...
                  when Thread =>
                     ...
                     if Is_Left_Less (The_Key, NZRT) then
                        Node := NZRT.all.LL;
                        ...
                        loop
                           ...
                           if Is_Left_Less (The_Key, Node) then
                              ...
                              Node.all.Balance := -1;
                              Node             := Node.all.LL;
                           elsif Is_Right_Less (Node, The_Key) then
   ...
   begin
      ...
      loop
         if Is_Left_Less (Of_The_Key, Node) then
            case Node.all.Link_Kind (Left) is
               when Child =>
                  Node := Node.all.LL;
               when Thread =>
         ...
         elsif Is_Right_Less (Node, Of_The_Key) then
            case Node.all.Link_Kind (Right) is
               when Child =>
                  Node := Node.all.RL;
               when Thread =>
